...
class _CalendarPageState extends State<CalendarPage> {
  ...
  void initState() {
    ...
    var newDoc = FirebaseFirestore.instance.collection('testCollection').doc();
    newDoc.set({"A": "B"});
  ...
  Widget dayBuilder(
    ...
    if (date.compareTo(currentDay) > 0) {
      ...
      if (CurrentSession.currentProfile.goodDays.any((element) =>
          element.day == date.day && element.month == date.month)) {
        backgroundColor = Colors.blue[200]!;
      } else if (CurrentSession.currentProfile.badDays.any((element) =>
          element.day == date.day && element.month == date.month)) {
